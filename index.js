...
function looksLikeVenue(text) {
  ...
  const bad = ["yes","no","okay","ok","sure","oo","hindi","di","wala","nalang","dont know","don't know","tbd","not yet","maybe","baka","siguro"];
...
function parseDetailBlob(text) {
  ...
  for (const p of datePat) { const m = text.match(p); if (m) { result.date = m[0]; break; } }
  ...
  for (const line of lines) {
    ...
    if (/\b(at|sa|venue|location|place|held|in|city|hall|hotel|resort|park|church|chapel|resto|restaurant|barangay|brgy|bgy|qc|manila|makati|taguig|pasig|cavite|laguna|batangas|bulacan|pampanga|rizal|paranaque|las pinas|muntinlupa|caloocan|malabon|valenzuela|navotas|marikina|pasay|pateros)\b/i.test(lo)) {
      if (!result.venue) result.venue = line; continue;
    }
    if (looksLikeName(line) && !result.name) { result.name = line; continue; }
  }
  return result;
}
...
app.post("/webhook", async (req, res) => {
  ...
  for (const entry of body.entry) {
    for (const event of entry.messaging) {
      ...
      if (!sessions[uid]) { sessions[uid] = { step: "idle" }; await sendWelcomeCard(uid); continue; }
      if (event.postback) await handlePostback(uid, event.postback.payload);
      else if (event.message && !event.message.is_echo) await handleMessage(uid, event.message.text || "");
    }
  }
});
...
async function askForDetails(uid) {
  ...
  await sendText(uid, "Almost there! Please send the following details in one message:\n\n" + needed + "\n\nYou can type them all together, e.g.:\nMaria Santos, Aling Nena, July 20 2025, Taguig City");
}
  ...
  switch (s.step) {
    ...
    case "collect_details": {
      ...
      if (parsed.date && !looksLikeDate(parsed.date)) {
        await sendText(uid, "I could not read the date clearly. Please use a format like July 20, 2025 or 07/20/2025.");
...
async function sendOccasionCards(uid) {
  ...
  row1.push({ title: "Others", subtitle: "Gender Reveal, Baby Shower & more", image_url: OCCASION_IMAGES["Others"], buttons: [{ type: "postback", title: "Select Others", payload: "OCC_Others" }] });
...
async function sendOthersSubCards(uid) {
  await callAPI({
    ...
    message: {
      ...
      quick_replies: OTHERS_SUB.map(o => ({ content_type: "text", title: o.title, payload: "OTHERSUB_" + encodeURIComponent(o.title) })),
    },
  });
}
...
async function sendText(uid, text) { await callAPI({ recipient: { id: uid }, message: { text } }); }
